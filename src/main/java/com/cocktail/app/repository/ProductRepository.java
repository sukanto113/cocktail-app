...
public interface ProductRepository extends CrudRepository<Product, Long> {
    ...
    @Query("SELECT p.id, p.category_id, c.name as category_name, p.name, p.picture, p.ingredients, p.method  FROM product AS p LEFT JOIN category as c on p.category_id=c.id")
    Iterable<ProductWithCategory> findProductsWithCategoryName();
